 ...
 *
 * Given the query, it can retrieve information of a specific device based on the UUID or URN or browse local network to fetch all responding UPNP devices.
 ...
 * @package Smalot\SSPDClient
 */
class SSDP
{
    /**
     * Callable method to fetch all responding UPNP devices on LAN.
     *
     ...
     */
    public function getAllDevices($json = false, $timeout = 2, $mx = null)
    {
    ...
    /**
     * Callable method to fetch all devices which are responding as root devices.
     *
     ...
     */
    public function getAllRootDevices($json = false, $timeout = 2, $mx = null)
    {
    ...
    /**
     * Callable method to launch a custom LAN search for a specific device or service given an URN description
     *
     ...
     */
    public function getDevicesByURN($urn, $json = false, $timeout = 1, $mx = null)
    {
    ...
    /**
     * Callable method to look for a device with a given UUID.
     *
     * Following standards, an UUID value SHOULD be unique, like a MAC address.
     * However, it's still possible to encounter an UUID conflict.
     ...
     */
    public function getDeviceByUUID($uuid, $json = false, $timeout = 1, $mx = null)
    {
        $device = null;
     ...
     * @return array
     */
    public function parseDeviceResponse(array $device)
    {
        $response = base64_decode($device['RESPONSE']);
        $headers = [];
        $content = '';

        if (($position = preg_split('/(\n\n|\r\n\r\n)/m', $response, 2, PREG_SPLIT_OFFSET_CAPTURE)) !== false) {
            if (($matches = preg_split('/(\r?\n)/', $position[0][0], 0, PREG_SPLIT_NO_EMPTY)) !== false) {
    ...
    {
        $devices = self::_search_request('"ssdp:discover"', $st, $timeout, $mx);
        $devices = array_merge($devices, self::_search_request('ssdp:discover', $st, $timeout, $mx));
     ...
     * @return array|null Array of devices or null if no devices are detected
     */
    private static function _search_request($man, $st, $timeout, $mx = null)
    {
        if (is_null($mx)) {
            $mx = $timeout;
        }
        ...
        while (@socket_recvfrom($socket, $response, 1024, MSG_WAITALL, $_tmp, $_tmp)) {
            ...
            // Response analysis.
            foreach ($keys as $key) {
                preg_match("/\s+$key\s*:\s*(.*)/i", $response, $tmp);
